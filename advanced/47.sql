...
SELECT orders.employee_id, last_name, all_orders, count(order_id) AS late_orders, ROUND((count(order_id) / all_orders::float)::numeric, 2) AS percent_last_orders FROM orders
